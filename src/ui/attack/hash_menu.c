...
MENU *hash_menu_get() {
  if (!s_hash_menu) {
    ...
    render_full_page_error_exit(
        stdscr, 0, 0, "Hash menu is not initialized. Call hash_menu_init() first.");
  }
...
bool hash_menu_init(WINDOW *win) {
  ...
  s_hash_menu_sub_win = derwin(win, hash_config_len, 32, 2, 1); // Create a sub-window for the menu

  list_menu_init(win,
                 hash_menu_choices,
                 hash_config_len,
                 &s_hash_menu_choices_items,
                 &s_hash_menu,
                 &s_hash_menu_sub_win);
  return true;
}
