...
paradox_form_field_get(int index) {
    return paradox_form_field[index];
}

FIELD**
paradox_form_field_get_all() {
    return paradox_form_field;
}
...
WINDOW*
paradox_form_sub_win_get() {
    return paradox_form_sub_win;
}
...
paradox_form_validate_all_fields() {
    ...
    for (int i = 0; i < paradox_form_field_metadata_len; i++) {
        ...
        if (result == E_INVALID_FIELD) {
            all_valid = false;
            display_field_error(paradox_form_sub_win, paradox_form_field_get(i), i,
                                max_label_length, longest_max_length_pad,
...
paradox_form_init(WINDOW* win, int max_y, int max_x) {
    ...
    for (unsigned short i = 0; i < paradox_form_field_metadata_len; ++i) {
        ...
        snprintf(string_buffer, paradox_form_field_metadata[i].max_length + 1, "%hu",
        ...
        set_field_buffer(paradox_form_field[i], 0,
    ...
    paradox_form_field[paradox_form_field_metadata_len] = create_button_field(
        form_submit_button_text, // Button label
        paradox_form_field_metadata_len + BH_FORM_Y_PADDING + 1, BH_FORM_X_PADDING);
...
paradox_form_render(WINDOW* win, int max_y, int max_x) {
    ...
    for (unsigned short i = 0; i < paradox_form_field_metadata_len; ++i) {
        mvwprintw(paradox_form_sub_win, i + BH_FORM_Y_PADDING, BH_FORM_X_PADDING,
                  paradox_form_field_metadata[i].label);
        mvwprintw(paradox_form_sub_win, i + BH_FORM_Y_PADDING, BH_FORM_X_PADDING + max_label_length,
                  ": [");
        mvwprintw(paradox_form_sub_win, i + BH_FORM_Y_PADDING,
                  BH_FORM_X_PADDING + max_label_length + BH_FORM_FIELD_BRACKET_PADDING + 1
                      + calculate_longest_max_length(paradox_form_field_metadata,
                                                     paradox_form_field_metadata_len, true)
                      + BH_FORM_FIELD_BRACKET_PADDING,
                  "]");
    }

    set_current_field(paradox_form, paradox_form_field[0]);
    update_field_highlighting(paradox_form, paradox_form_field_metadata_len + 1,
                              (unsigned short[]){paradox_form_field_metadata_len}, 1);
...
paradox_form_handle_input(WINDOW* win, int ch, double* collision_probability,
    ...
    switch (ch) {
        ...
        case KEY_DOWN: {
            ...
            if (result == E_INVALID_FIELD) {
                display_field_error(
                    paradox_form_sub_win, current, current_index, max_label_length,
                    longest_max_length_pad,
                    calculate_form_max_value(paradox_form_field_metadata[current_index].max_length),
                    true);

            } else {
                clear_field_error(paradox_form_sub_win, current_index, max_label_length,
                                  longest_max_length_pad);
            }
            ...
            if (current_index < paradox_form_field_metadata_len) {
                ...
                set_field_buffer(paradox_form_field_get(paradox_form_field_metadata_len), 0,
        ...
        case KEY_BACKSPACE:
        case 127: form_driver(paradox_form, REQ_DEL_PREV); break;
        case KEY_DC:
            if (current_index < paradox_form_field_metadata_len) {
